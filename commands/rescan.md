...
Use the **spec-rescan** skill to incrementally update the existing `knowledge-base/`:
find changed source files (via git), map them to the affected KB docs, and refresh only
those — preserving unrelated content. If there is no existing KB, fall back to `/spec-kit:scan`.
