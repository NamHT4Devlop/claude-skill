---
description: Whole-repo security audit — sweep entry points for OWASP-style vulns (injection, authz/IDOR, secrets, exposure, AI) grounded in CodeGraph + KB. Read-only.
...
surface (entry points via KB `03-entry-points`/`09-auth-security`/`11-api-docs` + CodeGraph), then
