...
1. **Ground everything in the Knowledge Base.** Load `knowledge-base/` from the repo
   ...
   If `knowledge-base/` is missing, tell the user to run `/spec-kit:scan` first, or
...
sub-agents** via the `Task` tool (see `agents/` — `codebase-analyzer`,
`impact-detector`, `business-flow-tracer`). If you don't spawn agents, do all three
