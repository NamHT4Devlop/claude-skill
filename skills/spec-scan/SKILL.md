...
> extension), prefer `/spec-kit:rescan` to update it. Only do a full scan for a new repo
...
`16-architecture-patterns.md` is the **guardrail** consumed by `/spec-kit:build` and
`/spec-kit:review`. Make Section 6 ("Architecture Invariants — DO NOT BREAK") a numbered,
...
valuable files (04, 05, 10, 13, review-skills) and suggest running `/spec-kit:build` next.
